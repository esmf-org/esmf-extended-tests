...
The main purpose of this example is to demonstrate the use of CMake for ESMF applications written in Fortran. The code is accompanied by `CMakeLists.txt` and `cmake/FindESMF.cmake` files, explicitly using CMake module mode.

> [!IMPORTANT]
> The `cmake/FindESMF.cmake` requires that the `ESMFMKFILE` environment variable is set and points to the `esmf.mk` file fo the desired ESMF installation. Spack loaded installations automatically satisfy this requirement.
